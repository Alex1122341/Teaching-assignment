  ...
  async function openSessionForm(existing = null) {
    ...
    $('session-form').onsubmit = async e => {
      ...
      if(UCVM.admin(currentUser)){
        const checks=assignments.map(a=>({a,f:facultyDirectory.find(x=>String(x.__id)===String(a.ucid||''))})).filter(x=>x.f).map(x=>({...x,av:facultyAssignmentAvailability(x.f,date,start,end,existing?.id||'')})).filter(x=>x.av.available!==true);
        if(checks.length){const msg=checks.map(x=>`- ${x.a.name}: ${assignmentAvailabilityDetail(x.av,date,start,end)}`).join('\n');if(!confirm(`Faculty availability warning for ${date} ${start}-${end}:\n\n${msg}\n\nThis checks both AFC and overlapping live timetable courses. AFC reasons are intentionally hidden here. Save this assignment anyway?`))return;}
      }
      const next = {
