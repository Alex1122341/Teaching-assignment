(() => {
  ...
  async function openSessionForm(existing = null) {
    ...
    $('session-form').onsubmit = async e => {
      ...
      try {
        ...
        if(canEditInstructor)await updateDerivedIndexes([{before:existing||null,after:savedNext}]);
   ...
   * Every render re-reads live session data through pageSessions(), so a session
   * whose date/time/topic changed is never shown from a stale snapshot. Closing
   * the panel only hides it; the toolbar button stays for as long as the current
   * role has outstanding required work. */
  function mountWorkQueue() {
    const queue = window.UCVM_WORK_QUEUE, workflow = window.UCVM_SESSION_WORKFLOW;
    if (!queue || !workflow || !document.body) return null;
    let controller = null, refreshing = false;
    ...
    // Opening Work navigates to the real Select Sessions context for the target
    // session rather than showing another read-only information modal.
    const openWorkItem = async ({ sessionId }) => {
      try {
        const target = (pageSessions() || []).find(row => String(row.id) === String(sessionId));
        const date = String(target?.date || '').slice(0, 10);
        if (date && typeof ensureSessionsForDates === 'function') await ensureSessionsForDates([date]);
        $('cal-list-btn')?.click();
        const row = await waitFor(() => document.querySelector(`[data-session-id="${String(sessionId).replace(/["\\]/g, '')}"]`));
        if (!row) { toast('That session is not visible in the current timetable view.', true); return; }
        row.click();
        const edit = await waitFor(() => document.getElementById('detail-edit'));
        if (!edit) { toast('The session editor is unavailable for this account.', true); return; }
        edit.click();
      } catch (error) { toast(error.message, true); }
    };
    const refresh = () => {
      ...
      refreshing = true;
      try {
        const role = UCVM.role(pageProfile()?.role);
        const view = queue.buildViewModel({
          sessions: pageSessions() || [],
          role,
          workflow,
          context: window.UCVM_WORK_QUEUE_CONTEXT || {}
        });
        view.role = role;
        if (!controller) controller = queue.createController({ document, host: document.body, onOpen: openWorkItem });
