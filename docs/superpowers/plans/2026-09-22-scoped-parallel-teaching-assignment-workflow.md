...
- group `id` is immutable and canonical: `^[a-z][a-z0-9_-]{0,63}# PAWS P3.1 Grouped HICC/VISC Teaching Assignment Workflow Implementation Plan
...
- export/test deterministic `submissionDocumentId(academicYearKey, groupId, hiccUid)`
  using the locked `ta-sub-v1__...` component encoding from the design
...
The submission locator is deterministically derived from the session's canonical
`academicYear`, group, and responsible HICC via the locked
`submissionDocumentId(...)` helper. It is internal-only and must not enter the
...
and trusted metadata; T7 owns Firestore enforcement and package-transition
security.
...
Do not make Firestore Rules reproduce JavaScript URL encoding.

Every P3.1-owned Working session must carry trusted
`teachingAssignmentSubmissionId`. HICC/VISC cannot mutate it. Rules load the
package directly through that locator and fail closed unless:
...
Missing/mismatched package identity denies P3.1 HICC/VISC operations.
...
- `teachingAssignmentSubmissionId` resolves to the exact matching Academic Year/group/HICC package.

HICC/VISC cannot change any of the three trusted ownership/package locator fields.
...
Extend the package model with:
- `workingRevision` (initial 0)
- `submittedWorkingRevision`
- `viscApprovedWorkingRevision`.

Keep existing `revision` as the HICC review-round counter; do not overload it as
the Working-content generation.
...
- validate the companion package write with `getAfter()`;
- one atomic write touching multiple review-relevant rows in one package advances
  the generation once;
- note-only, roster-only, derived-field-only and timestamp-only writes do not
  advance it.

Freeze review-relevant content during `visc_review`.
Deny review-relevant writes in `submitted_to_adfad` and `adfad_finalized`
outside the later controlled ADFAD final-assignment path.

Allow HICC Submit/Resubmit only when the current package is ready and the state is
`draft`, `changes_requested`, or stale `visc_approved`:
- increment review `revision`;
- set `submittedWorkingRevision = workingRevision`;
- record current `reviewFingerprint`;
- clear prior VISC approval generation/fingerprint;
- move to `visc_review`.

Allow VISC Approve only when:
- status is `visc_review`;
- `workingRevision == submittedWorkingRevision`;
- the VISC leads the package group;
- approved fingerprint is copied from the submitted `reviewFingerprint`;
- `viscApprovedWorkingRevision = workingRevision`.

Allow HICC Final Submit only when:
- status is `visc_approved`;
- `workingRevision == submittedWorkingRevision == viscApprovedWorkingRevision`;
- `reviewFingerprint == viscApprovedFingerprint`;
- the actor is the responsible HICC.

A browser-computed fingerprint alone must never satisfy stale-review security.
Deny stale Final Submit even when a client supplies the old approved fingerprint.

- [ ] **Step 4: Contribution/note privacy**

ADC/DVM, LAB, HICC actor docs use exact source/actor rules.

For a contribution write, resolve the source session, then resolve its trusted
submission locator. Changing the safe `suggestions` array is review-relevant and
must use the same atomic `workingRevision + 1` companion rule. Note-only and
actor-display/timestamp-only changes do not advance the package generation.

The contribution document's `sessionId`, `sourceRole`, and `actorUid` are
immutable identity/provenance and must match the actor-scoped document identity.
