...
actor-scoped contribution IDs:

```js
encodeURIComponent(value).replace(/_/g, '%5F')
```

Encoded components therefore cannot contain the `__` delimiter, pre-encoded
percent signs cannot collide with raw input, and the final UTF-8 document ID
must not exceed 1500 bytes.

Identity inputs are canonical before encoding:
- `academicYearKey` uses the existing `YYYY-YY` Academic Year identity and
...
`teachingAssignmentSubmissionId`. Rules use that value as the direct package
...
```js
{
  ...
  submittedWorkingRevision,     // pinned on HICC Submit/Resubmit
  viscApprovedWorkingRevision,  // pinned on VISC Approve
  reviewFingerprint,
...
`revision` and `workingRevision` are separate counters.

- `revision` increments only when HICC explicitly submits/resubmits a VISC
  review round.
- `workingRevision` starts at 0 and advances exactly once per atomic write set
  that changes one or more review-relevant facts for the package.
- Every review-relevant session or contribution-suggestion mutation must include
  a companion package write from `workingRevision = N` to `N + 1`.
  Firestore Rules verify the companion state with `getAfter()`.
- Multiple review-relevant rows for the same package in one atomic write advance
  the package generation once.
- Concurrent writers based on the same generation cannot both succeed; a stale
  writer must re-read and retry.

Review-relevant session facts are the stable facts used by the T4 fingerprint:
Course, Subject, date, start/end, `timeUnknown`, type, room and Topic.
Session identity is immutable. Trusted package-membership changes are also
review-significant and are allowed only through high-trust configuration while
no active review/handoff is in progress.

For actor-scoped contributions, a change to the safe `suggestions` array is
review-relevant. Contributor note text, actor display text, timestamps, LAB
group/roster data, derived week/semester/course-name fields, DOE data and
...
- `draft`, `changes_requested` and `visc_approved` may receive authorized
  ...
  Editing a `visc_approved` package therefore makes its approval stale without
  fabricating a VISC Push Back.
- review-relevant edits are denied while `visc_review`; it is a frozen review
  snapshot. Non-review-relevant LAB roster work remains independent.
- review-relevant edits are denied in `submitted_to_adfad` and
...
- clears the prior VISC-approved generation/fingerprint.

VISC Approve:
- requires `status == visc_review`;
- requires `workingRevision == submittedWorkingRevision`;
- records `viscApprovedWorkingRevision = workingRevision`;
- copies `reviewFingerprint` to `viscApprovedFingerprint`.
...
authorization proof of freshness.

A stale `visc_approved` package remains visibly stale until its HICC explicitly
resubmits it, preserving the existing T4 state-machine behavior.
...
10. HICC cannot Final Submit unless current server-enforced `workingRevision` equals both the submitted and latest VISC-approved Working revisions, with matching review/approved fingerprints as an additional consistency check.
11. A review-relevant edit atomically advances `workingRevision`; stale VISC approval therefore cannot authorize Final Submit until HICC resubmission and re-review.
