...
window.UCVM_CALENDAR_SESSION_MAINTENANCE=(()=>{
 ...
 const allowed=new Set(['sessionId','course','courseName','year','semester','week','date','start','end','timeUnknown','type','topic','room','instructorNames','instructor']);
 const idOf=row=>String(row?.id||row?.__id||row?.sessionId||'').trim();
 ...
 async function load(db,collection){const snap=await db.collection(collection).get();return snap.docs.map(doc=>({id:doc.id,...doc.data()}));}
