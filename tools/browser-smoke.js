...
async function demoRoleSelection(cdp,rolePattern,label){
 const load=waitForEvent(cdp,'Page.loadEventFired');
 const change=await cdp.send('Runtime.evaluate',{expression:`(()=>{const select=document.querySelector('#ucvm-pages-demo-toolbar select');if(!select)return{ok:false,reason:'toolbar select missing'};const option=[...select.options].find(item=>new RegExp(${JSON.stringify(rolePattern)},'i').test(item.textContent||''));if(!option)return{ok:false,reason:'role option missing'};select.value=option.value;select.dispatchEvent(new Event('change',{bubbles:true}));return{ok:true,uid:option.value,text:option.textContent}})()`,returnByValue:true});
...
async function verifyDemoRoleSwitching(cdp){
 const faculty=await demoRoleSelection(cdp,'·\\s*faculty\\s*
 const target=await newTarget(debugPort),cdp=await connectCdp(target.webSocketDebuggerUrl);
