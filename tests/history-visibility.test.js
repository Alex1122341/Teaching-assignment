...
test('timetable My Changes forces self-only scope even for administrators', () => {
  ...
  assert.match(panel, /UCVM\.logs\(content,\s*\{[^}]*scope:\s*'self'/s);
...
test('self history aggregates actor account AFC request and related teaching records', () => {
  ...
  assert.match(source, /where\('sessionId','==',d\.id\)/);
...
test('Firestore self history permits own actors and related records without granting global history', () => {
  ...
  assert.match(rules, /function historyReader\(d\)\{return historyAll\(\) \|\| \(ready\(\) && d\.get\('changedBy',''\) == request\.auth\.uid\);\}/);
  assert.match(rules, /function selfSessionHistory\(d\)/);
  assert.match(rules, /function selfFacultyHistory\(d\)/);
  assert.match(rules, /resource\.data\.get\('targetUid',''\) == request\.auth\.uid/);
  assert.match(rules, /match \/afc_audit\/\{id\} \{allow read: if ready\(\)/);
  assert.match(rules, /match \/change_request_audit\/\{id\}[\s\S]*resource\.data\.get\('requestId',''\)/);
});
