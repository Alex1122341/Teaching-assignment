...
test('multi-edit save passes stable session ids into conflict review',async()=>{
 ...
 const api=timetable(['saveSelectedChanges'],{window,firestoreSafeSessionPatch:row=>row,canEdit:()=>true,canSelectSessions:()=>true,capabilities:()=>({canEditInstructor:true,canEditCourseFields:true}),selectionRole:()=> 'adc',$:()=>({dataset:{},classList:{add(){},remove(){},toggle(){}},textContent:'',disabled:false}),selectedSessionOriginals:new Map([['stable-id',original]]),sessionSelection:{ids:()=>['stable-id']},readSelectionRows:()=>[changed],facultyDirectory:[{__id:'f1'}],firebase:{firestore:{FieldValue:{serverTimestamp:()=> 'SERVER'}}},ensureSessionsForDates:async()=>{},buildSelectionLabRosterPlans:()=>({plans:[],errors:[]}),currentUser:{uid:'admin1'},getTimetableDoeRuntime:()=>({adapter:{prepareSession:async(_before,after)=>({session:after,calculationRecords:[],doeChanges:[]})},repository:{stageCalculationRecord:()=>{}}}),doeAuditChanges:()=>[],confirmSchedulingChanges:rows=>{reviewed=rows;return null},toast:()=>{}});await api.saveSelectedChanges();assert.deepEqual(Array.from(reviewed,row=>row.id),['stable-id']);
});
