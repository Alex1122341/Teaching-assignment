...
(()=>{
 ...
 let user=null,profile=null,faculty=[],sessions=[],facultyById=new Map(),facultyUnsub=null,sessionUnsub=null,pendingExtra=null,roleAppendQueued=false,serverDoeRows=[],serverDoeYear='',serverDoeLoaded=false,serverDoeLoading=false,serverDoeError='';
 ...
 function ensureDoeView(){
  ...
  tab.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===tab));['lookup','summary','roles','doe-rules','sessional','database','history'].forEach(id=>$(`${id}-view`)?.classList.add('hidden'));sec.classList.remove('hidden');loadDoeList().catch(error=>console.error(error));renderDoeList()};
 ...
 function roleFactsFromRow(row){
  const roleType=String(row.querySelector('.ucvm-role-type')?.value||'Other').trim(),scope=String(row.querySelector('.ucvm-role-assignment')?.value||'').trim(),notes=String(row.querySelector('.ucvm-role-notes-input')?.value||'').trim();
  const facts={roleType,notes};
  if(roleType==='VISC')facts.subjectKey=scope;else facts.courseCode=scope;
  return facts;
 }
 function roleRowHtml(r={},options={}){
  const type=ROLE_TYPES.includes(r.type)?r.type:'Other',legacy=options.legacy===true,scope=r.assignment||r.courseCode||r.subjectKey||'',legacyDoe=num(r.doeCredit);
 ...
 function wireRoleRows(section){
  section.querySelectorAll('.ucvm-role-remove').forEach(b=>b.onclick=()=>b.closest('.ucvm-managed-role-row')?.remove());
  section.querySelectorAll('.ucvm-managed-role-row[data-legacy="0"]').forEach(row=>{for(const input of row.querySelectorAll('.ucvm-role-type,.ucvm-role-assignment'))input.addEventListener('change',()=>previewRoleRow(row));row.querySelector('.ucvm-role-assignment')?.addEventListener('input',()=>{clearTimeout(row._doeTimer);row._doeTimer=setTimeout(()=>previewRoleRow(row),250)});previewRoleRow(row)});
  const add=section.querySelector('#ucvm-add-managed-role');if(add)add.onclick=()=>{const box=section.querySelector('#ucvm-managed-role-rows');box.insertAdjacentHTML('beforeend',roleRowHtml({type:'Course Coordinator'}));wireRoleRows(section)};
 }
 async function enhanceEditor(form){
  const id=String(form.elements?.namedItem('ucid')?.value||'').trim(),key=id||'__new__';if(form.dataset.ucvmDoeEnhanced===key&&form.querySelector('#ucvm-doe-role-section'))return;form.dataset.ucvmDoeEnhanced=key;const f=facultyById.get(id)||{},year=doeListAcademicYear();let override=window.UCVM_FACULTY_DOE.override(f);
  if(id&&apiDoeReady()){try{const worksheet=await window.UCVM_DOE_API.getFacultyWorksheet(id,year),target=worksheet?.target||{};if(target.targetId||String(target.source||'').startsWith('legacy_'))override={value:num(target.overrideDoe),reason:String(target.overrideReason||''),notes:String(target.overrideNotes||'')}}catch(error){console.warn('DOE target lookup failed; using legacy migration fallback.',error)}}
 ...
 async function saveExtras(p){
  const u=auth.currentUser;if(!u||!p?.id)return;const roles=p.after.roles||[],beforeOverride=p.before.override||null,afterOverride=p.after.override||null,overrideChanged=JSON.stringify(beforeOverride)!==JSON.stringify(afterOverride);
  if(roles.length||overrideChanged){if(!window.UCVM_DOE_API?.isConfigured?.())throw Error('DOE API is required to save DOE assignments or targets.')}
  ...
  if(roles.length||overrideChanged){serverDoeLoaded=false;await loadDoeList({force:true})}
 }
