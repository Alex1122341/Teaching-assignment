...
(()=>{
 ...
 if(page!=='faculty-admin.html'||!window.UCVM||typeof firebase==='undefined')return;
 ...
 function doeRow(f,scheduled){
  const s=summary(f),fixed=num(s?.sourceNonTimetableTeachingDOE),sourceAssigned=num(s?.assignedTeachingDOE),sched=scheduled.get(String(f.__id))||0,base=fixed!==null?fixed+sched:sourceAssigned,roleAdj=managedRoleDOE(f),assigned=base===null?(roleAdj?roleAdj:null):base+roleAdj,contract=contractDOE(f),ov=num(f?.doeOverride2026_27?.value),target=ov!==null?ov:contract,diff=target!==null&&assigned!==null?target-assigned:null;
 ...
 function overrideNote(f){const o=f?.doeOverride2026_27;if(!o||num(o.value)===null)return'—';return [o.reason,o.notes].filter(Boolean).join(' · ')||'Office override'}
