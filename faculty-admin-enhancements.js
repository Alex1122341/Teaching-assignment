...
(()=>{
 ...
 function ensureDoeView(){
  ...
  tab.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===tab));['lookup','summary','roles','database','history'].forEach(id=>$(`${id}-view`)?.classList.add('hidden'));sec.classList.remove('hidden');loadDoeList().catch(error=>console.error(error));renderDoeList()};
