...
(()=>{
 ...
 function assignmentCredit(a){return num(a?.doeCredit)}
 ...
 async function ensureReplacementPeople(){
  ...
  // Read the sanitized people index instead of listing /users.
...
function routedOfficeActionHtml(r,currentOffice){
 ...
 async function finalizeRoutedRequest(id,officeOverride=''){
  ...
  if(workflow.hasFacultyChange){const [privateSnap,sourceSnap]=await Promise.all([db.doc(`${PRIVATE_REQUESTS}/${id}`).get({source:'server'}),db.doc(`${SESSIONS}/${request.sessionId}`).get({source:'server'})]);if(!privateSnap.exists||!sourceSnap.exists)throw Error('Required private Faculty/session data is missing.');facultySource={id:sourceSnap.id,...sourceSnap.data()};resolved=await resolveRoutedReplacement(privateSnap.data(),request);preflight=await routedFacultyPreflight(request,facultySource,privateSnap.data(),resolved);const warningText=preflight.warnings.length?'\n\nWARNING — availability/conflict checks:\n- '+preflight.warnings.join('\n- '):'';if(approvalScheduling.requiresOverride(preflight.check)){if(!confirm(`TIMETABLE CONFLICT DETECTED${warningText}\n\nOverride and apply this Faculty change? The override will be audited.`))throw Error('Approval cancelled.');conflictOverride={...approvalScheduling.overrideAudit({uid:user.uid,name:me?.name||'ADFA administrator'},preflight.check.conflicts),confirmedAt:stamp()}}else if(!confirm(`Apply the fully approved Faculty change to the live timetable?${warningText}`))throw Error('Approval cancelled.');if(!doeApiReady('saveSessionChange'))throw Error(doeApiUnavailable());const requestedAfter={...facultySource,...preflight.plan.sourcePatch,approvalRequestId:id,approvalRevision:request.revision};serverSavedFaculty=await doeApi.saveSessionChange({academicYear:facultySource.academicYear||'',sessionId:request.sessionId,afterSession:requestedAfter,trigger:'approved_routed_faculty_change'})}
 ...
 async function swapImpactHtml(r,current){
  const arr=assignedArray(current),fromId=String(r.fromFaculty?.facultyId||''),fromName=norm(r.fromFaculty?.name),idx=arr.findIndex(a=>(fromId&&String(a.ucid||a.facultyId||'')===fromId)||(fromName&&norm(a.name)===fromName)),out=idx>=0?arr[idx]:null,outF=resolveFaculty(r.fromFaculty),inF=resolveFaculty(r.toFaculty),outAv=availabilityFor(outF,current.date,current.start,current.end,current.id,current.timeUnknown===true),inAv=availabilityFor(inF,current.date,current.start,current.end,current.id,current.timeUnknown===true);
  if(!out||!inF)throw Error('Faculty transfer preview inputs are unavailable.');if(!doeApiReady('previewFacultyTransfer'))throw Error(doeApiUnavailable());
  const preview=await doeApi.previewFacultyTransfer({academicYear:current.academicYear||'',sessionId:current.id,assignmentId:String(out.assignmentId||`${current.id}--assignment--${idx+1}`),incomingFacultyId:String(inF.__id)});
  const creditText=preview.sessionDoeCredit===preview.incomingDoeCredit?fmtDoe(preview.incomingDoeCredit):`${fmtDoe(preview.sessionDoeCredit)} → ${fmtDoe(preview.incomingDoeCredit)}`;
  return `<div class="workflow-impact-title">DOE & schedule checks</div><div class="workflow-credit">Session DOE credit: ${creditText} · ${esc(ymd(current.date))} ${esc(current.start||'—')}–${esc(current.end||'—')}</div><div class="workflow-impact-grid"><div class="workflow-person"><div class="workflow-person-name">Outgoing · ${esc(r.fromFaculty?.name||facultyName(outF))}</div><div class="workflow-metric">${doeProjectionText(preview.outgoing?.currentAssignedDoe??null,preview.outgoing?.projectedAssignedDoe??null,preview.outgoing?.effectiveTargetDoe??contractTeachingDoe(outF))}</div>${availabilityHtml(outAv,current.start,current.end)}</div><div class="workflow-person"><div class="workflow-person-name">Incoming · ${esc(r.toFaculty?.name||facultyName(inF))}</div><div class="workflow-metric">${doeProjectionText(preview.incoming?.currentAssignedDoe??null,preview.incoming?.projectedAssignedDoe??null,preview.incoming?.effectiveTargetDoe??contractTeachingDoe(inF))}</div>${availabilityHtml(inAv,current.start,current.end)}</div></div>`;
 }
 async function routedSwapImpactHtml(r,current,privateRecord,resolved){
  const change=privateRecord?.assignmentChange||{},arr=assignedArray(current),index=Number(change.assignmentIndex)||0,out=arr[index]||null,outRef={facultyId:String(change.from?.facultyId||out?.ucid||''),name:r.currentFacultyName||out?.name||''},outF=resolveFaculty(outRef),inF=resolved?.faculty||null,outAv=availabilityFor(outF,current.date,current.start,current.end,current.id,current.timeUnknown===true),incomingName=resolved?.name||r.proposedFacultyName||'Replacement Faculty';
  if(resolved?.special){const incoming=`<div class="workflow-person"><div class="workflow-person-name">Incoming · ${esc(incomingName)}</div><div class="workflow-metric">DOE/AFC/availability checks will be completed by ADFA when the specific Faculty record is resolved.</div><div class="workflow-check unknown">Specific Faculty identity not yet assigned.</div></div>`;return `<div class="workflow-impact-title">DOE, AFC & schedule checks</div><div class="workflow-impact-grid"><div class="workflow-person"><div class="workflow-person-name">Outgoing · ${esc(r.currentFacultyName||out?.name||facultyName(outF))}</div><div class="workflow-metric">Current DOE remains visible until a specific replacement is resolved.</div>${availabilityHtml(outAv,current.start,current.end)}</div>${incoming}</div>`}
  if(!out||!inF)throw Error('Faculty transfer preview inputs are unavailable.');if(!doeApiReady('previewFacultyTransfer'))throw Error(doeApiUnavailable());
  const preview=await doeApi.previewFacultyTransfer({academicYear:current.academicYear||'',sessionId:current.id,assignmentId:String(out.assignmentId||`${current.id}--assignment--${index+1}`),incomingFacultyId:String(inF.__id)}),inAv=availabilityFor(inF,current.date,current.start,current.end,current.id,current.timeUnknown===true),creditText=preview.sessionDoeCredit===preview.incomingDoeCredit?fmtDoe(preview.incomingDoeCredit):`${fmtDoe(preview.sessionDoeCredit)} → ${fmtDoe(preview.incomingDoeCredit)}`;
  const incoming=`<div class="workflow-person"><div class="workflow-person-name">Incoming · ${esc(incomingName)}</div><div class="workflow-metric">${doeProjectionText(preview.incoming?.currentAssignedDoe??null,preview.incoming?.projectedAssignedDoe??null,preview.incoming?.effectiveTargetDoe??contractTeachingDoe(inF))}</div>${availabilityHtml(inAv,current.start,current.end)}</div>`;
  return `<div class="workflow-impact-title">DOE, AFC & schedule checks</div><div class="workflow-credit">Session DOE credit: ${creditText} · ${esc(ymd(current.date))} ${esc(current.start||'—')}–${esc(current.end||'—')}</div><div class="workflow-impact-grid"><div class="workflow-person"><div class="workflow-person-name">Outgoing · ${esc(r.currentFacultyName||out?.name||facultyName(outF))}</div><div class="workflow-metric">${doeProjectionText(preview.outgoing?.currentAssignedDoe??null,preview.outgoing?.projectedAssignedDoe??null,preview.outgoing?.effectiveTargetDoe??contractTeachingDoe(outF))}</div>${availabilityHtml(outAv,current.start,current.end)}</div>${incoming}</div>`;
 }
 async function editImpactHtml(r,current){
  const patch=r.patchPublic||r.patch||{},date=patch.date||current.date,start=patch.start||current.start,end=patch.end||current.end;
  if(!doeApiReady('previewSessionChange'))throw Error(doeApiUnavailable());
  const preview=await doeApi.previewSessionChange({academicYear:current.academicYear||'',sessionId:current.id,afterSession:{...current,...patch},trigger:'approval_preview'}),impactByFaculty=new Map((preview.facultyImpacts||[]).map(row=>[String(row.facultyId),row]));
  const rows=assignedArray(preview.session||current).map(a=>{const f=resolveFaculty({facultyId:a.ucid||a.facultyId,name:a.name}),impact=f?impactByFaculty.get(String(f.__id)):null,av=availabilityFor(f,date,start,end,current.id,current.timeUnknown===true&&start===current.start&&end===current.end),credit=assignmentCredit(a);return `<div class="workflow-person"><div class="workflow-person-name">${esc(a.name||facultyName(f))}</div><div class="workflow-metric">Session DOE: <strong>${credit===null?'Unrated':fmtDoe(credit)}</strong> · ${doeProjectionText(impact?.currentAssignedDoe??null,impact?.projectedAssignedDoe??null,impact?.effectiveTargetDoe??contractTeachingDoe(f))}</div>${availabilityHtml(av,start,end)}</div>`}).join('');
  return `<div class="workflow-impact-title">DOE & proposed-time conflict checks</div><div class="workflow-credit">Proposed session: ${esc(ymd(date))} ${esc(start||'—')}–${esc(end||'—')}. DOE and timetable conflicts are checked against the server-authoritative preview.</div><div class="workflow-edit-impact">${rows||'<div class="workflow-check unknown">No assigned faculty were found to check.</div>'}</div>`;
 }
...
async function hydrateApprovalImpacts(){if(!isAdfaApprover())return;
 ...
 async function approveRequest(id){
  ...
  const snap=await db.doc(`${SESSIONS}/${r.sessionId}`).get({source:'server'});if(!snap.exists)return toast('The session no longer exists. Reject or review this request manually.',true);const current={id:snap.id,...snap.data()};
