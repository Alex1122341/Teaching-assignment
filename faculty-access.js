...
const config=window.UCVM_FIREBASE_CONFIG;
 ...
 const retiredRole=r=>rawRole(r)==='other_office';
 const role=r=>({owner:'adfa_general',administrator:'adfa_regular',adfa_general:'adfa_general',adfa_regular:'adfa_regular',admin:'adfa_regular',editor:'faculty',viewer:'faculty'}[rawRole(r)]||rawRole(r));
 ...
 const label=r=>({developer:'Developer',owner:'Owner',adfa_general:'Owner',administrator:'Administrator',adfa_regular:'Administrator',admin:'Administrator',other_office:'Other Office (retired)',adc:'ADC/DVM',lab:'LAB',hicc:'HICC',visc:'VISC',faculty:'Faculty',editor:'Faculty',viewer:'Faculty'}[rawRole(r)]||rawRole(r));
 ...
 async function ready(user,p){if(window.__ucvmSessionGuard?.blocked)return false;if(!p?.active)throw Error('This account is inactive.');if(retiredRole(p))throw Error('This account role has been retired. Contact an administrator to assign a supported role.');if(p.mustChangePassword){location.replace('password.html');return false}installLandingReset();await linkFacultyIdentity(user,p);if(scheduleAdminLanding(user,p))return false;return true}
 ...
 async function logs(container,{includeFaculty=false,profile=null}={}){
  ...
  const {auth,db}=init(),user=auth.currentUser,canReadAll=historyAll(profile);const PAGE_SIZE=20;let entries=[],loading=false;const cursors={session:null,faculty:null,account:null};const exhausted={session:false,faculty:!includeFaculty,account:false};
  ...
  async function page(collection,key){if(exhausted[key])return;let q=db.collection(collection);if(!canReadAll)q=q.where('changedBy','==',user.uid);q=q.orderBy('changedAt','desc').limit(PAGE_SIZE);if(cursors[key])q=q.startAfter(cursors[key]);const s=await q.get();entries.push(...s.docs.map(d=>normalizeEntry({id:d.id,...d.data()},key)));cursors[key]=s.docs.at(-1)||cursors[key];if(s.size<PAGE_SIZE)exhausted[key]=true}
