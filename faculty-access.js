...
const config=window.UCVM_FIREBASE_CONFIG;
 ...
 async function logs(container,{includeFaculty=false,profile=null,personalOnly=false}={}){
  ...
  function changeCell(e){
    const changes=e.changes||[],group=window.UCVM_AUDIT_DETAILS?.group(changes,false)||{visible:changes.slice(0,3),remaining:Math.max(0,changes.length-3)};
    if(!changes.length){
      if(e.source==='afc')return `<span>${esc(actionLabel(e.action))}</span>`;
      return'<span>Legacy entry; detailed before/after values were not recorded.</span>';
    }
    const visible=group.visible.map(changeLine).join('');
    if(!group.remaining)return visible;
    return `${visible}<details class="audit-change-more"><summary>Show ${group.remaining} more change${group.remaining===1?'':'s'} <span aria-hidden="true">⌄</span></summary>${changes.slice(3).map(changeLine).join('')}</details>`;
  }
  function render(){
    const q=$('audit-search').value.toLowerCase(),kind=$('audit-kind').value;
    const rows=entries.filter(e=>(!q||JSON.stringify(e).toLowerCase().includes(q))&&kindMatch(e,kind));
    $('audit-body').innerHTML=rows.map(e=>`<tr><td>${esc(time(e))}</td><td>${esc(e.changedByName||e.changedBy||'Unknown')}<small>${esc(e.changedByEmail||'')}</small></td><td>${esc([e.course,e.date,e.topic].filter(Boolean).join(' · ')||e.facultyName||e.facultyId||e.targetName||e.targetUid||e.sessionId||e.requestId||e.recordId||'Record')}</td><td>${esc(actionLabel(e.action))}</td><td>${changeCell(e)}</td></tr>`).join('')||'<tr><td colspan="5">No matching changes.</td></tr>';
  }
  const addRows=(snapshot,key)=>{
    for(const doc of snapshot.docs){
      const entry=normalizeEntry({id:doc.id,...doc.data()},key);
      const identity=`${key}:${doc.id}`;
      if(entries.some(row=>row.__historyIdentity===identity))continue;
      entry.__historyIdentity=identity;entries.push(entry);
    }
  };
  async function page(collection,key){
    if(exhausted[key])return;
    let q=db.collection(collection).orderBy('changedAt','desc').limit(PAGE_SIZE);
    if(cursors[key])q=q.startAfter(cursors[key]);
    const snap=await q.get();addRows(snap,key);cursors[key]=snap.docs.at(-1)||cursors[key];if(snap.size<PAGE_SIZE)exhausted[key]=true;
  }
  async function queryRelated(collection,key,field,operator,value){
  ...
  async function more(){
    ...
    try{
      if(canReadAll){
        await page('session_change_log','session');
