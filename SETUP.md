...
7. Confirm the DOE App Service `GET /api/health` endpoint is healthy and its `ALLOWED_ORIGINS` includes `https://red-cliff-04871ca0f.5.azurestaticapps.net`.
...
7. The `main` push starts the **Azure Static Web Apps** workflow. Its `validate_and_build` job runs the full test suite again, requires the two production Actions variables, generates and verifies the `tester-teaching` Firebase client configuration plus the HTTPS DOE API base URL, builds `.deploy-static`, adds the canonical `staticwebapp.config.json`, and uploads an immutable `azure-production-${{ github.sha }}` Actions artifact. Missing or invalid production client configuration fails the build before any artifact is uploaded. The `main` push does not deploy production.
